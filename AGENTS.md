...
<!-- agents:begin delegation-brief@0a5a04 -->
...
- Commit **and push** before dispatching — a worktree branches from local HEAD, so uncommitted work is invisible to the agent.
- Agents never commit; the integrator makes every commit on `main`: snapshot the agent's branch with hooks off, `git merge --squash` it (never `--no-ff` — that keeps a `wip` commit in history), read the diff (that is the code review), commit through the hooks with a real message from the agent's proposed one, push, then close the leaves.
