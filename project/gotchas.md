...
**Under machine-wide load, the session and print golden tests deadlock rather than fail, and a `swift test` run then never ends.** Seen with nine agents from another project plus three SleepyHollow suites running at once (load average 150–270): `sleepy _host` helpers parked their main thread in `semaphore_wait_trap`, their `shot --session` / `find --session` clients waited 7–11 minutes past a 60 s budget, `pdf` timed out in `NSPrintOperation`, and `takeSnapshot` returned `WKErrorDomain 1` — a different test each run. Filed as job issue MN69b (a session client must own a deadline). Rule from Ben: **a test run is capped at five minutes**; past that it is a deadlock — kill it by pid (`lsof -a -p <pid> -d cwd` tells you which checkout's run it is; never `pkill -f`) and count it as red. The pre-commit hook enforces the cap. Do not add `yes` spinners (`scripts/flake-hunt.sh N 12`) on a machine that is already loaded; measure under the ambient load and say so.
