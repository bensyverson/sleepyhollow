...
**Golden tests that shell out to the `sleepy` binary contend for real WebKit resources under Swift Testing's default parallel execution.** `swift test --quiet` runs ~11+ golden tests as concurrent subprocesses, each spinning up its own headless `WKWebView`; on a loaded/sandboxed dev machine this pushes some invocations past the default 30s budget (exit 3), non-deterministically. Reproduced: the pre-existing `LoadGoldenTests` (owned by `cli-scaffold`, untouched here) fails intermittently in the full 131-test suite but passes 100% of the time run alone (`swift test --quiet --skip Capture`, 112 tests, ~3.5s) or filtered to its own suite. Capture family golden tests (`CaptureGoldenTests`) mitigate this locally by passing `--budget 60000` on every subprocess invocation, which made them reliably green across repeated full-suite runs — but this doesn't fix `LoadGoldenTests` or any other family's golden suite, and the problem will worsen as more wave-1 families add golden tests. Worth a real fix at the harness level (e.g. serialize golden-test execution, or raise `GoldenBinary`'s effective default budget) before wave-1 fully lands.
