...
4. **The unknown-option suggester proposes flags that don't exist on the verb.** `sleepy shot --full` → *"Did you mean '--fill'?"* — `--fill` is not a `shot` option (fill is a different subcommand); the right suggestion was `--full-page`. An agent will take that suggestion literally and burn a turn.
   ...
   > **Corrected 2026-08-28 (leaf dBRXS).** The stated cause is wrong, and it would have sent the next reader to the wrong seam. `--fill` *is* a `shot` option: it comes from the shared `LoadFlagOptions` group, whose `--click`/`--fill`/`--submit` one-shot action flags every loading verb carries (`sleepy shot --help` lists it). The suggester — ArgumentParser's own `unknownOptionMessage`, not our `nearestVerb` — was already constrained to the invoked verb's argument set; verified against the built binary, `sleepy shot --selectr` proposes nothing at all, because `--selector` is a `query` flag and `shot` has none. So there was nothing to constrain. The real defect is the one the wrong cause hid: `--full` should have been a `shot` flag, and `--fill` was simply its nearest legitimate neighbour. Fixed by aliasing `--full` onto `--full-page` (and `-o` onto `--out`, on the shared `OutOption` so every artifact verb gets it); a golden test now pins the property the bug report reached for — any `Did you mean '--x'?` must name a flag that appears in that verb's own `--help`.
